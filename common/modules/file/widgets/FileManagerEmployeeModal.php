 ...
 */
class FileManagerEmployeeModal extends InputWidget
{
	/**
	 * @var string
	 */
...
JS;
		...
		if(count($this->model_db->{$this->relation_name})){
			...
			foreach ($via_table_elements as $via_element){
				$file = File::findOne($via_element->file_id);
				if($file === null){ return false;}
				$this->selected[] = $file->id;
				$isImage = $file->getIsImage() ? "filemanager-sortable-item-image" : "";

				$this->selected_items_sortable[$via_element->file_id]['content'] = '<div data-id="' . $file->id . '" class="filemanager-sortable-item ' . $isImage . '"><img src="' . $file->getSrc('small') . '"/><i class="filemanager-sortable-item-icon glyphicon glyphicon-file"></i><span class="filemanager-sortable-item-name">' . Html::encode($file->title) . '</span><span class="filemanager-sortable-item-remove"><i class="glyphicon glyphicon-remove"></i></span></div>';
	...
	public function run()
    {
        ...
        return $this->render('FileManagerEmployeeModal', [
			'model'     => $model,
			'widget'    => $this,
			'form'      => $this->form,
